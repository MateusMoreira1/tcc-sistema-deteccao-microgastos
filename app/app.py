...
try:
    from database import excluir_conta
    EXCLUSAO_DISPONIVEL = True
except ImportError:
    EXCLUSAO_DISPONIVEL = False

try:
    from database import buscar_perfil_usuario, atualizar_perfil_usuario
...
if 'renda_salva' not in st.session_state: st.session_state.renda_salva = 5000.0
if 'threshold_salvo' not in st.session_state: st.session_state.threshold_salvo = 50.0
if 'arquivo_processado_assinatura' not in st.session_state: st.session_state.arquivo_processado_assinatura = None
...
if st.session_state.user is None:
                ...
                with st.spinner("Conectando..."):
                    ...
                    if user:
                        ...
                        if PERFIL_DISPONIVEL:
                            ...
                            if perfil.get('renda_mensal') is not None:
                                st.session_state.renda_salva = perfil['renda_mensal']
                            if perfil.get('threshold_microgasto') is not None:
                                st.session_state.threshold_salvo = perfil['threshold_microgasto']
    ...
    if EXCLUSAO_DISPONIVEL:
        with st.expander("Excluir conta permanentemente"):
            try:
                df_conta_atual = buscar_historico_supabase(st.session_state.user.id)
                qtd_microgastos = len(df_conta_atual) if not df_conta_atual.empty else 0
            except Exception:
                qtd_microgastos = None

            st.markdown(
                f"Esta ação apaga a conta **{st.session_state.user.email}** e "
                "todos os microgastos vinculados a ela no banco de dados. "
                "Não há como desfazer ou recuperar esses dados depois de confirmado."
            )
            if qtd_microgastos is not None:
                if qtd_microgastos > 0:
                    st.markdown(f"Você tem **{qtd_microgastos} microgasto(s)** salvos na nuvem que serão apagados junto com a conta.")
                else:
                    st.markdown("Você ainda não tem nenhum microgasto salvo na nuvem — apenas a conta de acesso será removida.")

            st.write(f"Para confirmar, digite seu e-mail de cadastro (`{st.session_state.user.email}`):")
            confirmacao_email = st.text_input(
                "Confirmação de e-mail", key="confirmacao_exclusao_email",
                label_visibility="collapsed", placeholder=st.session_state.user.email
            )
            pode_excluir = confirmacao_email.strip().lower() == st.session_state.user.email.strip().lower()

            if confirmacao_email and not pode_excluir:
                st.caption("O e-mail digitado não corresponde à sua conta.")

            if st.button("Excluir conta e todos os dados", type="primary", disabled=not pode_excluir):
                with st.spinner("Excluindo conta..."):
                    sucesso, erro = excluir_conta(st.session_state.user.id)
                    if sucesso:
                        st.session_state.user = None
                        ...
                        st.error(f"Erro ao excluir conta: {erro}")
    ...
    with t1:
        ...
        with col_s1:
            ...
            renda = st.number_input("Renda Mensal (R$)", value=st.session_state.renda_salva, step=500.0)
            limite = st.number_input("Threshold de Microgasto (R$)", value=st.session_state.threshold_salvo, step=10.0)
            if st.button("Confirmar Configuração", type="primary"):
                st.session_state.perfil_configurado = True
                st.session_state.renda_salva = renda
                st.session_state.threshold_salvo = limite
                if PERFIL_DISPONIVEL:
                    atualizar_perfil_usuario(st.session_state.user.id, renda, limite)
        ...
        with col_s2:
            ...
            if st.session_state.perfil_configurado:
                ...
                if file:
                    assinatura_atual = f"{file.name}_{file.size}"
                    ja_processado = st.session_state.arquivo_processado_assinatura == assinatura_atual
                    ...
                    if not ja_processado:
                        with st.spinner("Minerando dados via Regex..."):
                            try:
                                ...
                                st.session_state.arquivo_processado_assinatura = assinatura_atual
                        ...
                        st.success("✅ Arquivo já processado. Suas edições na Auditoria estão preservadas — envie um novo arquivo se quiser reprocessar do zero.")
            else:
